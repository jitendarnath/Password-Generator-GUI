...
def save_pass():
    ...
    with open("D:\\Programming\\Tkinter\\records.txt","a") as f:
...
def records():
    ...
    top.fileName = filedialog.askopenfilename \
                (filetypes=(("Python Stuff", ".txt"), \
                            ("All files","*.*"))) 
    text1 = open(top.fileName).read()
    T = Text(top, height=15, width=60)
    T.pack()                 
    T.insert(END,text1) 
...
account_name.grid(row=0,columnspan=2,sticky="we",pady=5,ipadx=28,ipady=3)
account_name.insert(0," Enter account name")

username = Entry(right_sub_frame2,font=("Helvetica",15),bg='#002447',fg='PeachPuff')
username.grid(row=1,column=0,pady=5,ipadx=28,ipady=3)
...
m1.add_separator()
m1.add_command(label="Open", command=save_pass)
